...
public class FMLNetworkHandler {
    ...
    private final Map<NetLoginHandler, Integer> loginStates = Maps.newHashMap();
    private final Map<ModContainer, NetworkModHandler> networkModHandlers = Maps.newHashMap();

    private final Map<Integer, NetworkModHandler> networkIdLookup = Maps.newHashMap();
    ...
    private void handleClientConnection(NetLoginHandler netLoginHandler, MinecraftServer server, SocketAddress address, String userName) {
        if (!loginStates.containsKey(netLoginHandler)) {
            if (handleVanillaLoginKick(netLoginHandler, server, address, userName)) {
                ...
                netLoginHandler.completeConnection(MCPCConfig.getString("settings.fml-missing-message", "You do not have FML installed, you cannot connect to this server")); // MCPC+
                return;
            } else {
        ...
        switch (loginStates.get(netLoginHandler)) {
            ...
            case MISSING_MODS_OR_VERSIONS:
                netLoginHandler.completeConnection(MCPCConfig.getString("settings.fml-missing-mods-message", "The server requires mods that are absent or out of date on your client")); // MCPC+
                loginStates.remove(netLoginHandler);
                break;
            case FML_OUT_OF_DATE:
                netLoginHandler.completeConnection(MCPCConfig.getString("settings.fml-outdated-message", "Your client is not running a new enough version of FML to connect to this server")); // MCPC+
                loginStates.remove(netLoginHandler);
                break;
            default:
                netLoginHandler.completeConnection(MCPCConfig.getString("settings.fml-other-error-message", "There was a problem during FML negotiation")); // MCPC+
                loginStates.remove(netLoginHandler);
                break;
        }
    }
