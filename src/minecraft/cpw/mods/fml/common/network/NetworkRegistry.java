...
import gilbertoscode.MCPCTotal.MCPCConfig;
...
public class NetworkRegistry {
     ...
     */
    private final Multimap<Player, String> activeChannels = ArrayListMultimap.create();
    /**
     * A map of the packet handlers for packets
     */
    private final Multimap<String, IPacketHandler> universalPacketHandlers = ArrayListMultimap.create();
    private final Multimap<String, IPacketHandler> clientPacketHandlers = ArrayListMultimap.create();
    private final Multimap<String, IPacketHandler> serverPacketHandlers = ArrayListMultimap.create();
    /**
     * A linked set of registered connection handlers
     */
    private final Set<IConnectionHandler> connectionHandlers = Sets.newLinkedHashSet();
    ...
    private final List<IChatListener> chatListeners = Lists.newArrayList();
    ...
    void playerLoggedIn(EntityPlayerMP player, NetServerHandler netHandler, INetworkManager manager) {
        ...
        for (IConnectionHandler handler : connectionHandlers) {
            handler.playerLoggedIn(player, netHandler, manager);
        }
    }
    ...
    void handleCustomPacket(Packet250CustomPayload packet, INetworkManager network, NetHandler handler) {
        if ("REGISTER".equals(packet.channel)) {
            handleRegistrationPacket(packet, handler.getPlayer());
            handleBukkitRegistrationPacket(packet, ((NetServerHandler) (handler)).getPlayerB()); // MCPC+
        } else if ("UNREGISTER".equals(packet.channel)) {
            handleUnregistrationPacket(packet, handler.getPlayer());
            handleBukkitUnregistrationPacket(packet, ((NetServerHandler) (handler)).getPlayerB()); // MCPC+
        } else {
            handlePacket(packet, network, handler.getPlayer());
    ...
    private void handleBukkitRegistrationPacket(Packet250CustomPayload packet, CraftPlayer player) {
        try {
            ...
            for (String channel : channels.split("\0")) {
                if (MCPCConfig.connectionLogging) {
