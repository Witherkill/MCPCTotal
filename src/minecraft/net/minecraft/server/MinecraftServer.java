...
import gilbertoscode.MCPCTotal.FMLLogJLineBreakProxy;
// MCPC+ end
...
public abstract class MinecraftServer implements ICommandSender, Runnable, IPlayerUsage
{
        ...
        if (!this.worldIsBeingDeleted)
        {
            ...
            for (int j = 0; j < this.worlds.size(); ++j)
            {
                ...
                if (worldserver != null)
                {
                    if (!par1)
                    {
                        FMLLog.info("Salvando chunks para o level \'" + worldserver.getWorldInfo().getWorldName() + "\'/" + worldserver.provider.getDimensionName());
                    }
    ...
    public void stopServer() throws MinecraftException   // CraftBukkit - added throws
    {
        if (!this.worldIsBeingDeleted)
        {
            ...
            {
                FMLLog.info("Salvando Players");
                this.serverConfigManager.saveAllPlayerData();
                this.serverConfigManager.removeAllPlayers();
            }

            FMLLog.info("Salvando Mundos");
    ...
    {
        FMLLog.info(StringUtils.stripControlCodes(par1Str));
    }
